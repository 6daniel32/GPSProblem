...
def Stack(): #una lista se puede comportar como una pila xD
...
class FIFOQueue(Queue):
    ...
    def pop(self): #todo el pop es equivalente a "return A.pop(0)" pero está hecho más eficiente por el if ese raro (buffer de extracción de 5 elementos)
        e = self.A[self.start] #sacar el elemento numero "start" de la lista
        self.start += 1
        if self.start > 5 and self.start > len(self.A) / 2: #esto es una forma de recorrer más eficiente
            self.A = self.A[self.start:]
            self.start = 0
        return e

class CostQueue(Queue): #crear nueva cola de costes pero que incluya heurística. Comparar ciudades visitadas y distancia recorrida
    ...
    def extend(self, items): #ordenar lista "items" por coste
        self.A.extend(items)
        self.A.sort(key=lambda e: e.path_cost)
...
class HeuristicCostQueue(Queue): #crear nueva cola de costes pero que incluya heurística. Comparar ciudades visitadas y distancia recorrida
    ...
    def extend(self, items): #ordenar lista por coste + heuristica
        self.A.extend(items)
        self.A.sort(key=lambda e: e.path_cost+self.problem.h(e))
        #print(self.problem.h(self.A[0]))
